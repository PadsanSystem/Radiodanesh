...
function showsublinks() {
	$i = 0; $res = "";
	$sres = dbquery("SELECT * FROM ".DB_PREFIX."site_links WHERE link_position>='2' AND link_step='0' ORDER BY link_order DESC");
	if (dbrows($sres) != 0) {
		while($sdata = dbarray($sres)) {
			if (checkgroup($sdata['link_visibility'])) {
				if ($sdata['link_url']!="---") {				
					...
					$result=dbquery("SELECT * FROM ".DB_PREFIX."site_links WHERE link_position>='2' AND link_step='".$sdata['link_id']."'");
					if(dbrows($result)!=0){
						?>
						<li class="dropdown">
							<a href="<?php echo $sdata['link_url']; ?>" class="dropdown-toggle" data-toggle="dropdown"><?php echo $sdata['link_name']; ?>&nbsp;<span class="caret"></span></a>
							<ul class="dropdown-menu" role="menu">
								<?php
									while($data=dbarray($result)){
										?>
										<li class="text-right"><a href="<?php echo BASEDIR.$data['link_url']; ?>"><?php echo $data['link_name']; ?>&nbsp;&nbsp;<span class="<?php echo $data['link_image']; ?>"></span></a></li>
										<?php
									}
									?>
							</ul>
						...
						if (strstr($sdata['link_url'], "http://") || strstr($sdata['link_url'], "https://")) {
							echo "<li><a href='".BASEDIR.$sdata['link_url']."'>".$sdata['link_name']." <span class='glyphicon glyphicon-home'></span></a></li>";
						}else{
							echo "<li><a href='".BASEDIR.$sdata['link_url']."'>".$sdata['link_name']."</a></li>";
						}
					}
				}
				$i++;
