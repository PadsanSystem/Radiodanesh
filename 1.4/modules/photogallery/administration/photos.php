...
define("PHOTODIR", PHOTOGALLERY_IMAGES.(!SAFEMODE ? "album_".$album_id."/" : ""));
define("PHOTODIR1", PHOTOGALLERY_IMAGES.(!SAFEMODE ? "album_thumbnail/" : ""));
	...
	if (is_uploaded_file($_FILES['photo_pic_file']['tmp_name'])) {
		...
		if (!preg_match("/^[-0-9A-Z_\.\[\]]+$/i", $photo_pic['name'])) {
			...
			if ($imagefile[0] > $settings['photo_max_w'] || $imagefile[1] > $settings['photo_max_h']) {
				...
				createthumbnail($imagefile[2], $photo_dest.$photo_file, $photo_dest1.$photo_thumb1, $settings['thumb_w'], $settings['thumb_h']);
