...
if (!isset($cat_id)) {
	...
	if ($rows != 0) {
		...
		while ($data = dbarray($result)) {
			...
			echo "<td align='right' valign='top' width='50%' class='tbl'><a href='".FUSION_SELF."?cat_id=".$data['download_cat_id']."'>".$data['download_cat_name']."</a> <span class='small2'>($num)</span>";
		...
		if (checkgroup($cdata['download_cat_access'])) {
			...
			if ($rows != 0) {
				...
				while ($data = dbarray($result)) {
					...
					echo "<td width='40%' class='tbl2'><b>".$locale['413']."</b> ".$data['download_version']."</td>\n</tr>\n<tr>\n<td width='30%' class='tbl2'><b>".$locale['414']."</b> ".showdate("l j F Y", $data['download_datestamp'])."</td>\n";
