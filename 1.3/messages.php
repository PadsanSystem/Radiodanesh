...
if (isset($_POST['send_message'])) {
	...
	$subject = trim(stripinput($_POST['subject']));
	...
	if (iADMIN && isset($_POST['chk_sendtoall']) && isNum($_POST['msg_to_group'])) {
		...
		if ($msg_to_group == "101" || $msg_to_group == "102" || $msg_to_group == "103") {
			...
			while ($data = dbarray($result)) {
				if ($data['user_id'] != $userdata['user_id']) {
					$result2 = dbquery("INSERT INTO ".$db_prefix."messages (message_to, message_from, message_subject, message_message, message_read, message_datestamp, message_folder) VALUES('".$data['user_id']."','".$userdata['user_id']."','".$subject."','".$message."','0','".time()."','0')");
		...
		if ($data['user_id'] != $userdata['user_id']) {
			if ($msg_settings['pm_inbox'] == "0" || ($data['message_count'] + 1) <= $msg_settings['pm_inbox']) {
				$result = dbquery("INSERT INTO ".$db_prefix."messages (message_to, message_from, message_subject, message_message, message_read, message_datestamp, message_folder) VALUES('".$data['user_id']."','".$userdata['user_id']."','".$subject."','".$message."','0','".time()."','0')");
				$send_email = isset($data['pm_email_notify']) ? $data['pm_email_notify'] : $msg_settings['pm_email_notify'];
				if ($send_email == "1") sendemail($data['user_name'],$data['user_email'],$settings['siteusername'],$settings['siteemail'],$locale['625'],$data['user_name'].$locale['626']);
			} else {
				$error = "2";
	...
	if (!$error) {
		...
		if ($my_settings['pm_save_sent']) {
			...
			$result = dbquery("INSERT INTO ".$db_prefix."messages (message_to, message_from, message_subject, message_message, message_read, message_datestamp, message_folder) VALUES ('".$userdata['user_id']."','".$outbox_user."','".$subject."','".$message."','1','".time()."','1')");
		}
	}
...
if (!isset($msg_send) && !isset($msg_read) && $folder != "options") {
	...
	echo "<tr>\n<td align='right' width='100%' class='tbl'><a href='".FUSION_SELF."?msg_send=0'>".$locale['401']."</a></td>\n";
	...
	echo "<tr>\n<td colspan='2' class='tbl1'>".$message_message."</td>\n</tr>\n";
	echo "</table>\n";
	echo "<table border='0' cellpadding='0' cellspacing='0' width='100%'>\n";
	echo "<tr>\n<td colspan='2' class='tbl'><a href='".FUSION_SELF."?folder=$folder'>".$locale['432']."</a></td>\n";
	echo "<td align='right' class='tbl'>\n";
	if ($folder == "inbox") echo "<input type='submit' name='reply' value='".$locale['439']."' class='button'>\n";
	if ($folder == "inbox") echo "<input type='submit' name='save' value='".$locale['412']."' class='button'>\n";
	if ($folder == "archive") echo "<input type='submit' name='unsave' value='".$locale['413']."' class='button'>\n";
	...
	if (isset($_POST['send_preview'])) {
		$subject = trim(stripslashes(censorwords($_POST['subject'])));
		$message = trim(stripslashes(censorwords($_POST['message'])));
		...
		echo "<table cellpadding='0' cellspacing='1' width='100%' class='tbl-border'>\n<tr>\n";
		echo "<td class='tbl1'>".$message_preview."</td>\n</tr>\n";
		echo "</table>\n";
		closetable();
	} else {
		$subject = ""; $message = ""; $msg_send_state = ""; $msg_to_group = "";
		$msg_to_group_state = " disabled"; $sendtoall_chk = "";	
	}	
	...
	echo "<form name='inputform' method='post' action='".$PHP_SELF."?msg_send=0' onSubmit=\"return ValidateForm(this)\">\n";
	echo "<table cellpadding='0' cellspacing='1' width='100%' class='tbl-border'>\n";
	...
	if ($reply_message) {
		...
		echo "<td class='tbl1'>".$reply_message."</td>\n</tr>\n";
	}
	echo "<tr>\n<td align='right' class='tbl2' valign='middle' style='white-space:nowrap;'>".($reply_message ? $locale['433'] : $locale['422']).":</td>\n";
	echo "<td class='tbl1'><textarea id='message_reply' name='message' class='textbox' style='width:100%;'>".$message."</textarea></td>\n</tr>\n";
	if ($settings['tinymce_enabled'] == 1){
		...
		plugins:"emotions",
		mode : "exact",
		elements : "message_reply",
		theme_advanced_toolbar_align : "center"
		});
		</script>';
	}
	echo "</table>\n";
	echo "<table border='0' cellpadding='0' cellspacing='0' width='100%'><br>\n";
