...
if (!isset($cat_id)) {
	...
	if ($rows != 0) {
		...
		while ($data = dbarray($result)) {
			...
			echo "<td align='right' valign='top' width='50%' class='tbl'><a href='".FUSION_SELF."?cat_id=".$data['weblink_cat_id']."'>".$data['weblink_cat_name']."</a> <span class='small2'>( $num )</span>";
		...
		if (checkgroup($cdata['weblink_cat_access'])) {
			...
			if ($rows != 0) {
				...
				while ($data = dbarray($result)) {
					...
					echo "<tr>\n<td width='30%' class='tbl2'><b>".$locale['411']."</b> ".showdate("l j F Y", $data['weblink_datestamp'])."</td>\n";
