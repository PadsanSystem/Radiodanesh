...
if (dbrows($result) != 0) {
	if (isset($_POST['save'])) {
		...
		if (isset($article_id)) {
			$result = dbquery("UPDATE ".$db_prefix."articles SET article_cat='".$_POST['article_cat']."', article_subject='$subject', article_snippet='$body', article_article='$body2', article_allow_comments='$comments', article_allow_ratings='$ratings' WHERE article_id='$article_id'");
			redirect(FUSION_SELF.$aidlink."&status=su");
		} else {
			$result = dbquery("INSERT INTO ".$db_prefix."articles (article_cat, article_subject, article_snippet, article_article, article_name, article_datestamp, article_reads, article_allow_comments, article_allow_ratings) VALUES ('".$_POST['article_cat']."', '$subject', '$body', '$body2', '".$userdata['user_id']."', '".time()."', '0', '$comments', '$ratings')");
...
<td class='tbl'><textarea id='body1' name='body' cols='95' rows='5' class='textbox'>$body</textarea></td></tr>\n";
...
<td class='tbl'><textarea id='body2' name='body2' cols='95' rows='10' class='textbox'>$body2</textarea></td></tr>\n";
if ($settings['tinymce_enabled'] == 1){
