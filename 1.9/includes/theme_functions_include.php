		...
		while ($sdata = dbarray($sres)) {
			...
			if ($sdata['link_url'] != "---" && checkgroup($sdata['link_visibility'])) {
				...
				if (preg_match("!^(ht|f)tp(s)?://!i", $sdata['link_url'])) {
					...
					$res .= "<a href='".BASEDIR.$sdata['link_url']."'".$link_target." style='color:white'><li ".($li_class ? " class='".$li_class."'" : "").">".$sep."";
					$res .= "<span>".parseubb($sdata['link_name'], "b|i|u|color|img")."</span></li></a>";
				}
			}
		}
		$res .= "";
