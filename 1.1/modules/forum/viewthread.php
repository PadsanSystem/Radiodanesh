...
if (iMEMBER && $can_post && !$tdata['thread_locked'] && isset($_POST['postquickreply'])) {
	$flood = false;
	$message = stripslash(censorwords($_POST['message']));
	if ($message != "") {
		...
		if (!$flood) {
			...
			$subject = "پاسخ : ".$tdata['thread_subject'];
...
if ($rows != 0) {
	...
	while ($data = dbarray($result)) {
		...
		if ($data['post_edittime'] != "0") {
			$edituser = "<a href='".BASEDIR."profile.php?lookup=".$data['post_edituser']."'>".$data['edit_name']."</a>";
...
<a href='".BASEDIR."profile.php?lookup=".$data['user_id']."'>".$data['user_name']."</a><br>
<span class='alt'>".($is_mod ? $locale['userf1'] : getuserlevel($data['user_level']))."</span><br><br>\n";
		...
		echo "<span class='alt'>".$locale['504']."</span> ".showdate("l j F Y", $data['user_joined'])."</td>
<td>
...
<td align='left' class='tbl1'>\n";
		if (iMEMBER && $can_post) {
			if (!$tdata['thread_locked']) {
				...
				if (iMOD || iSUPERADMIN && $data['post_ip'] != "0.0.0.0" && file_exists(THEME."forum/ip.jpg")) echo "<img src='".THEME."forum/ip.jpg' alt='".$locale['570']."' title='".$data['post_ip']."' style='border:0px;'>\n";
			} else {
				if (iMOD || iSUPERADMIN) {
					echo "<a href='post.php?action=edit&amp;forum_id=$forum_id&amp;thread_id=".$data['thread_id']."&amp;post_id=".$data['post_id']."'><img src='".THEME."forum/edit.gif' alt='".$locale['568']."' style='border:0px;'></a>\n";
					if ($data['post_ip'] != "0.0.0.0" && file_exists(THEME."forum/ip.jpg")) echo "<img src='".THEME."forum/ip.jpg' alt='".$locale['570']."' title='".$data['post_ip']."' style='border:0px;'>\n";
				}
			}
		}
		echo "</td>
...
<tr>
<td valign='bottom' class='tbl1'>امضا :<p>".nl2br($data['user_sig'])."</td>
</tr>
<tr>
<td class='tbl1'>\n";
		if ($data['user_aim']) {
			echo "<a href='aim:goim?screenname=".str_replace(" ", "+", $data['user_aim'])."' target='_blank'><img src='".THEME."forum/aim.jpg' alt='".$data['user_aim']."' style='border:0px;'></a> ";
		}
		if ($data['user_icq']) {
			echo "<a href='http://icq.com/people/about_me.php?uin=".$data['user_icq']."' target='_blank'><img src='".THEME."forum/icq.jpg' alt='".$data['user_icq']."' style='border:0px;'></a> ";
		}
		if ($data['user_msn']) {
			echo "<a href='mailto:$data[user_msn]'><img src='".THEME."forum/msn.jpg' alt='".$data['user_msn']."' style='border:0px;'></a> ";
		}
		if ($data['user_yahoo']) {
...
$forum_list .= "</optgroup>\n";
echo "<td align='right' class='tbl'>".$locale['540']." <select name='jump_id' class='textbox' onChange=\"jumpForum(this.options[this.selectedIndex].value);\">$forum_list</select></td>\n";
...
<tr>
<td align='center' class='tbl1'><textarea id='messages1' name='message' cols='80' rows='7' class='textbox'></textarea>
</tr>";
