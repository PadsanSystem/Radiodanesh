...
if ($stage == "2" && $t == "n") {
	if (isset($_POST['publish'])) {
		...
		$result = dbquery("INSERT INTO ".$db_prefix."news (news_subject, news_cat, news_news, news_extended, news_name, news_datestamp, news_start, news_end, news_visibility, news_reads, news_allow_comments, news_allow_ratings) VALUES ('$news_subject', '$news_cat', '$news_body', '', '".$data['user_id']."', '".time()."', '0', '0', '0', '0', '1', '1')");
		$result = dbquery("DELETE FROM ".$db_prefix."submissions WHERE submit_id='$submit_id'");
...
<a href='index.php".$aidlink."'>".$locale['403']."</a></div><br>\n";
		...
		$news_body = stripslashes($submit_criteria['news_body']);
		$news_cat_opts = ""; $sel = "";
...
<td width='80%' class='tbl'><textarea id='news_body' name='news_body' cols='65' rows='10' class='textbox'>$news_body</textarea></td>
</tr>";
...
if ($stage == "2" && $t == "a") {
	if (isset($_POST['publish'])) {
		...
		$address_file = $data['submit_address_file'];
		$result = dbquery("INSERT INTO ".$db_prefix."articles (article_cat, article_subject, article_snippet, article_article, article_name, article_files, article_datestamp, article_reads, article_allow_comments, article_allow_ratings) VALUES ('$article_cat', '$article_subject', '$article_snippet', '$article_body', '".$data['user_id']."', '".$address_file."', '".time()."', '0', '1', '1')");
		$result = dbquery("DELETE FROM ".$db_prefix."submissions WHERE submit_id='$submit_id'");
...
<td width='80%' class='tbl'><textarea id='article_snippet' name='article_snippet' cols='65' rows='5' class='textbox'>$article_snippet</textarea></td>
</tr>";
if ($settings['tinymce_enabled'] == 1){
echo '<script type="text/javascript" src="'.INCLUDES.'/editor/tiny_mce.js"></script>
<script type="text/javascript">
...
<td width='80%' class='tbl'><textarea id='article_body' name='article_body' cols='65' rows='10' class='textbox'>$article_body</textarea></td>
</tr>";
if ($settings['tinymce_enabled'] == 1){
